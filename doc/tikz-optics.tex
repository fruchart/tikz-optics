...
\begin{key}{/tikz/optics/prism apex angle=\meta{angle} (initially \pgfkeysvalueof{/tikz/optics/prism apex angle})}
    La clé |prism apex angle| contrôle l'angle au sommet des trois prismes identiques. \meta{angle} est exprimé en degrés. Si \meta{angle} vaut |60|, les prismes sont des triangles equilatéraux.
...
\end{tikzpicture}  
  ...
  Plusieurs sous-clés permettent de spécifier des options en écrivant |dim arrow={|\meta{sous-clés}|}|.
...
\end{codeexample}
  ...
  La clé |label pos| permet de spécifier la position \meta{number} à laquelle le label doit être dessiné.
...
La manière dont l'oeil est dessiné peut-être modifiée en utilisant |optics eye={\meta{styles}}|.
...
Lorsqu'une pic |optics eye| est ajoutée au point |(0,0)|, l'œil dessiné est déplacé de la longueur |optics eye/shift| de manière à ce que les rayons lumineux ne touchent pas l'œil. Cette distance peut être modifiée en utilisant |optics eye={shift=\meta{length}}|.
Deux coordonnées |(\meta{name}-in)| et |(\meta{name}-center)| sont créées, où \meta{name} est le nom de la pic. Par exemple |\pic (eye) at (0,0) {optics eye};| produira les coordonnées |(eye-in)| [à |(0,0)|] et |(eye-center)|. Ces points sont séparés de |optics eye/shift| et leurs positions sont expliquées dans la figure ci-dessous.
